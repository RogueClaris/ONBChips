...
local CROSS_AUDIO = Engine.load_audio(_modpath.."cross.ogg")

function package_init(package) 
	package:declare_package_id("com.clarisAndKonst.card.VarSwordNeo")
	package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
	package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'N'})

	local props = package:get_card_props()
	props.shortname = "NeoVari"
	props.damage = DAMAGE
...
function take_cross_action(actor, props)
	...
	actor:card_action_event(action, ActionOrder.Involuntary)
	Engine.play_audio(CROSS_AUDIO, AudioPriority.Low)
end
	...
	action.action_end_func = function(self)
		...
		action2.action_end_func = function(self)
			...
			action3.action_end_func = function(self)
				local action4 = Battle.CardAction.new(actor, "PLAYER_SWORD")
				action4:set_lockout(make_animation_lockout())
				action4:add_anim_action(3, function()
					local hilt = action4:add_attachment("HILT")
					local hilt_sprite = hilt:sprite()
					hilt_sprite:set_texture(actor:get_texture())
					hilt_sprite:set_layer(-2)
					hilt_sprite:enable_parent_shader(true)
				...
				actor:card_action_event(action4, ActionOrder.Involuntary)
			end
		end
	end
end
