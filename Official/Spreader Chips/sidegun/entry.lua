...
function package_init(package) 
    package:declare_package_id("com.claris.card.Shotgun4")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'C', 'H', 'M', 'S', 'Y', '*'})

    local props = package:get_card_props()
    props.shortname = "Sidegun"
    props.damage = DAMAGE
...
function create_attack(user)
    ...
    spell.attack_func = function(self, other) 
		...
		local tile2 = self:get_current_tile():get_tile(Direction.Up, 1)
		...
		local tile3 = self:get_current_tile():get_tile(Direction.Down, 1)
