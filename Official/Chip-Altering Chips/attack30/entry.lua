...
function package_init(package) 
    package:declare_package_id("com.claris.card.AttackPlus2")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'*'})

    local props = package:get_card_props()
    props.shortname = "Atk+30"
    props.damage = 0
    props.time_freeze = false
    props.element = Element.Plus
    props.description = "+30 for selected atk chip"
	...
	package.filter_hand_step = function(in_props, adj_cards) 
        if adj_cards:has_card_to_left() and adj_cards.left_card.can_boost then
            adj_cards.left_card.damage = adj_cards.left_card.damage + 30
