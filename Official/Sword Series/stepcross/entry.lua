...
function package_init(package) 
    package:declare_package_id("com.claris.card.StepSword02")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'B', 'S', 'Y'})

    local props = package:get_card_props()
    props.shortname = "StepCros"
    props.damage = DAMAGE
    props.time_freeze = false
    props.element = Element.Sword
    props.description = "Cross atk 2sq ahead"
	props.limit = 1
end
			...
			if warp_tile and not warp_tile:is_edge() and not warp_tile:is_reserved({actor:get_id()}) then
				...
				self:add_anim_action(3,
					function()
						...
						local sword2 = create_slash(user)
						local tile = user:get_tile(user:get_facing(), 1)
						local props = sword:copy_hit_props()
						
						props.damage = DAMAGE * 2
						sword2:set_hit_props(props)
