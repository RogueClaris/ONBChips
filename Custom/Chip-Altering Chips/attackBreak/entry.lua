...
function package_init(package) 
    package:declare_package_id("com.claris.card.AttackPlusBreak")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'*'})

    local props = package:get_card_props()
    props.shortname = "Break+20"
    props.damage = 0
    props.time_freeze = false
    props.element = Element.Plus
    props.description = "+20 for selected brk chip"
	props.card_class = CardClass.Standard
	props.limit = 3
	props.can_boost = false
	
	package.filter_hand_step = function(in_props, adj_cards) 
        if adj_cards:has_card_to_left() and adj_cards.left_card.can_boost and adj_cards.left_card.element == Element.Break then
