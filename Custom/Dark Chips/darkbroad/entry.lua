...
function package_init(package) 
    package:declare_package_id("com.claris.dark.Sword02")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'Z'})

    local props = package:get_card_props()
    props.shortname = "DrkBroad"
    props.damage = DAMAGE
    props.time_freeze = false
    props.element = Element.Sword
    props.description = "SWEEP THEM ALONG"
...
function card_create_action(actor, props)
    ...
    action.execute_func = function(self, user)
		...
		self:add_anim_action(3,
			function()
				local sword = create_slash(user, "WIDE")
...
function create_slash(user, animation_state)
	...
	local sweep_dir = Direction.Up
	if user:get_current_tile():get_tile(Direction.Up, 1):is_edge() then
