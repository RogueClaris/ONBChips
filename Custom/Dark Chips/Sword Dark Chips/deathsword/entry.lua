...
function package_init(package) 
    package:declare_package_id("com.claris.dark.DeathSword")
    package:set_icon_texture(Engine.load_texture(_modpath.."icon.png"))
    package:set_preview_texture(Engine.load_texture(_modpath.."preview.png"))
	package:set_codes({'*'})

    local props = package:get_card_props()
    props.shortname = "DethSwrd"
    props.damage = DAMAGE
	...
	props.limit = 1
	props.long_description = "A defiant sword of darkness. If your HP contains the digit 1, deal additional damage!"
end
...
function create_slash(user)
	...
	spell.collision_func = function(self, other)
		if is_digit_in_num(1, user:get_health()) then
			other:set_health(other:get_health()-499)
		end
	end
...
function is_digit_in_num(digit, num) 
   num = math.floor(num) -- whole integers only
   local i = num
   
   while i >= 1 do
       local step = math.floor(i / 10)
       local frac = math.floor(i % 10)
       
       if frac == digit or (step == digit and step >= 1) then
           return true
       end
